...
**Architecture:** 复用现有抖店持久浏览器页面。在服务端创建最小的内存任务管理器，使用 `AbortController` 取消正在执行的 Playwright 页面；前端启动后轮询任务状态并调用暂停接口。
...
- [ ] 为自动化选项加入 `signal`，将暂停转为可识别的取消错误并返回列表页；提交后继续下一次配置，保持同一登录浏览器。
- [ ] 重跑相同测试，确认通过。
