...
export function App() {
  ...
  return (
    <div className="app-shell">
      ...
      {showUpdateDialog ? <div className="updateDialogBackdrop" onMouseDown={() => setShowUpdateDialog(false)}><div onMouseDown={(event) => event.stopPropagation()}><UpdateDialog onCheck={() => void updater.check()} onClose={() => setShowUpdateDialog(false)} onDownload={() => void updater.download()} onOpenRelease={() => void updater.openRelease()} onRestart={() => void updater.restart()} state={updater.state} /></div></div> : null}
